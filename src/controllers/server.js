...
const server = {
	...
	saveParam: function (type, index, value, params) {
		...
		if (JSON.stringify(customImageUpdateMethodConfig) !== "{}") {
			if ("images" != d.k) {
				...
				let msg = mode ? JSON.stringify(d) : JSON.stringify({
					context: {
						gridKey: encodeURIComponent(_this.gridKey),
		...
		if ('WebSocket' in window) {
			...
			_this.websocket.onopen = function () {
				...
				_this.retryTimer = setInterval(function () {
					mode ? _this.websocket.send("rub") : _this.websocket.send(JSON.stringify({
						scene: "health"
					}))
				}, 30000);
			}
			...
			_this.websocket.onmessage = function (result) {
				...
				if (type == 1) { //send 成功或失败
					...
					const dataCopy = data.data
					if (!dataCopy) return
					let item = JSON.parse(dataCopy);
					_this.wsUpdateMsg(item);
					let chang_data = JSON.parse(dataCopy)
	...
	multipleRangeShow: function (id, name, r, c, value) {
		...
		// 如果正在编辑，就显示“正在输入”
		if (value === 'enterEdit') {
			str = locale().edit.typing;
		}
		console.log($("#luckysheet-multipleRange-show-" + id).length > 0, '>0++++')
		if ($("#luckysheet-multipleRange-show-" + id).length > 0) {
			...
			$("#luckysheet-multipleRange-show-" + id + " .username ww-open-data").attr('openid', name);
			$("#luckysheet-multipleRange-show-" + id + " .username .typing").text(str);
